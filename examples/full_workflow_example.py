...
def test_gp_factor_mining(data):
    ...
    try:
        ...
        if not hasattr(ae, 'GpEngine'):
            ...
            print("  Note: Rebuild with 'maturin develop --release' to include GP features")
            return False
...
def test_persistence_system():
    ...
    try:
        ...
        if not hasattr(ae, 'PersistenceManager'):
            ...
            print("  Note: Rebuild with 'maturin develop --release' to include persistence features")
            return False
...
def test_meta_learning_system():
    ...
    try:
        ...
        if not hasattr(ae, 'MetaLearningAnalyzer'):
            ...
            print("  Note: Rebuild with 'maturin develop --release' to include meta-learning features")
            return False
        ...
        print(f"  ✓ Got recommendations (confidence: {recommendations.confidence_score:.2f})")
...
def main():
    ...
    if all_passed:
        ...
        print("\nNext steps:")
        print("1. Install in production: `pip install .` or `maturin develop --release`")
        ...
        print("1. Ensure Rust toolchain is installed: `curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh`")
        print("2. Install maturin: `pip install maturin`")
        print("3. Rebuild extension: `cd /path/to/alpha-expr && maturin develop --release`")
