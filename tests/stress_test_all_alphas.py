...
START_YEAR = 2010
END_YEAR = 2025
BACKTEST_BATCH_SIZE = 20  # factors per batch (20*16y*20MB ≈ 6.4GB slices, KeepAll DL ~2.4GB)
...
with open(summary_path, "w", newline="") as f:
    w = csv.writer(f)
    w.writerow(["factor", "ic_mean", "ic_ir", "sharpe", "ann_ret", "max_dd", "turnover", "rss_mb"])

    for name, r in results:
        ic = r.ic_mean
        w.writerow([
            name, f"{ic:.6f}", f"{r.ic_ir:.4f}", f"{r.sharpe_ratio:.4f}",
            f"{r.annualized_return:.4f}", f"{r.max_drawdown:.4f}",
            f"{r.turnover:.4f}", f"{rss_mb():.0f}",
        ])
...
print(f"\n{'='*60}")
print(f"Completed {len(results)}/{len(all_alphas)} factors in {total:.1f}s ({total/60:.1f}min)")
...
with open(rank_path, "w", newline="") as f:
    ...
    for rank, (name, ic, sharpe, ann_ret, mdd, turnover) in enumerate(scored, 1):
        w.writerow([rank, name, f"{ic:.6f}", f"{sharpe:.4f}", f"{ann_ret:.4f}", f"{mdd:.4f}", f"{turnover:.4f}"])
