...
data = {"close": prices, "returns": returns}
    ...
    if "ma_5" in window_results:
        ...
        for asset_idx in range(min(3, n_assets)):  # Check first 3 assets
            ...
            if np.sum(mask) > 0:
                ...
                if diff < 1e-10:
                    print(
                        f"  Asset {asset_idx}: MA5 calculation correct (max diff={diff:.2e})"
                    )
                else:
                    print(
                        f"  Asset {asset_idx}: MA5 calculation incorrect (max diff={diff:.2e})"
                    )
    ...
    if "cumulative_returns" in stateful_results:
        ...
        for asset_idx in range(min(3, n_assets)):
            ...
            if np.sum(mask) > 0:
                ...
                if diff < 1e-10:
                    print(
                        f"  Asset {asset_idx}: Cumulative sum correct (max diff={diff:.2e})"
                    )
                else:
                    print(
                        f"  Asset {asset_idx}: Cumulative sum incorrect (max diff={diff:.2e})"
                    )
